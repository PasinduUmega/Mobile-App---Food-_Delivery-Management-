    ...
    try {
      ...
      if (widget.ownerUserId != null) {
        ...
        final allowedOrderIds = orders
            .where((o) => o.storeId != null && storeIds.contains(o.storeId))
            .map((o) => o.orderId)
            .toSet();
        list = items.where((d) => allowedOrderIds.contains(d.orderId)).toList();
      }
      if (mounted) {
        setState(() {
          _deliveries = list;
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text('Could not load order: $e')));
      }
      return;
    }
    if (!mounted) return;
    final o = order;
    final captured = payments.any((p) => p.status == 'CAPTURED');
      ...
      body: _loading
          ...
          : ListView.builder(
              ...
              itemBuilder: (ctx, i) {
                ...
                final paid =
                    pay != null && pay.status.toUpperCase() == 'CAPTURED';
...
class _DeliveryEditDialogState extends State<_DeliveryEditDialog> {
  ...
  Future<void> _save() async {
    ...
    if (driverNameError != null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(driverNameError)));
      return;
    }
    ...
    if (phoneRequiredErr != null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(phoneRequiredErr)));
      return;
    }
    ...
    if (driverPhoneError != null) {
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text(driverPhoneError)));
      return;
    }
            ...
            : Column(
                ...
                children: [
                  ...
                  if (!(isNew && _availableOrders.isEmpty)) ...[
                    DropdownButtonFormField<int>(
                      initialValue: _selectedDriverUserId,
                      isExpanded: true,
