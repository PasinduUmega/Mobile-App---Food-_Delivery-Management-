...
/// Driver-focused list: assigned runs, order context, payment clarity, status updates.
class DriverDeliveriesScreen extends StatefulWidget {
  final User user;

  const DriverDeliveriesScreen({super.key, required this.user});
...
class _DriverDeliveriesScreenState extends State<DriverDeliveriesScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final list = await _api.listDeliveries();
      if (!mounted) return;
      list.sort((a, b) {
        int rank(String s) {
          switch (s) {
