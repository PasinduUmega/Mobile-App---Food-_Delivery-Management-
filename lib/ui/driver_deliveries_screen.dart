...
class _DriverDeliveriesScreenState extends State<DriverDeliveriesScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.readOnly) {
        // Admin read-only view: fetch all deliveries
        list = await _api.listDeliveries();
      } else {
        // CHANGED: use dedicated driver endpoint instead of fetching all
        // and filtering client-side. Calls GET /api/deliveries/driver/:userId
  ...
  Future<void> _setStatus(DeliveryInfo d, String status) async {
    ...
    try {
      ...
      // CHANGED: only sends status (+ coords if needed).
      // Driver can only update: PICKED_UP, OUT_FOR_DELIVERY, DELIVERED, CANCELLED
      // Backend rejects driverName/driverPhone from this endpoint now.
  ...
  // REMOVED: _claim() method deleted entirely.
  // Previously it sent driverName + driverPhone to PUT /api/deliveries/:id
  // to self-assign. The backend now rejects those fields from drivers —
  // assignment is admin-only via POST /api/orders/:id/assign.
  // Drivers just tap "Start pickup" which calls _setStatus(d, 'PICKED_UP').
      ...
      body: _loading
          ...
          : _error != null
              ...
              : ListView(
                  ...
                  children: [
                    // CHANGED: banner updated to reflect admin-assigns-driver flow
                    Card(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            const SizedBox(height: 8),
                            Text(
                              'Orders are assigned to you by an admin. '
                              'Tap "Start pickup" when you collect the bag, then update '
                              'status as the run progresses. '
                              'Each card shows the basket and payment method for handover.',
                              style: TextStyle(
                                color: cs.onSurfaceVariant,
                                height: 1.35,
                                fontSize: 13,
                    ...
                    if (_items.isEmpty)
                      ...
                      ..._items.map((d) {
                        final o = _orderById[d.orderId];
                        final pay = _paymentByOrderId[d.orderId];
                        final showCollectCod = !widget.readOnly &&
                            o != null &&
                              ...
                              children: [
                                ...
                                Wrap(
                                  ...
                                  children: [
                                    ...
                                    // CHANGED: was _claim(d) which sent driverName/Phone
                                    // to the backend (now rejected). Now just calls
                                    // _setStatus(d, 'PICKED_UP') directly.
                                    if (!widget.readOnly && d.status == 'PENDING')
                                      FilledButton.icon(
                                        onPressed: () =>
                                            _setStatus(d, 'PICKED_UP'),
