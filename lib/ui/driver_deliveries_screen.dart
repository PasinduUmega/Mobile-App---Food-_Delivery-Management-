...
class _DriverDeliveriesScreenState extends State<DriverDeliveriesScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      if (widget.readOnly) {
        list = await _api.listDeliveries();
      } else {
        list = await _api.listDriverDeliveries(userId: widget.user.id);
      }
      if (!mounted) return;
      list.sort((a, b) {
        int rank(String s) {
          ...
          builder: (_, scroll) => Padding(
            ...
            child: ListView(
              ...
              children: [
                ...
                _kv(
                  'Subtotal',
                  '${o.currency} ${o.subtotal.toStringAsFixed(2)}',
                ),
                _kv(
                  'Delivery fee',
                  '${o.currency} ${o.deliveryFee.toStringAsFixed(2)}',
                ),
                ...
                if (o.deliveryLatitude != null && o.deliveryLongitude != null)
                  _kv(
                    'Drop-off GPS',
                    '${o.deliveryLatitude!.toStringAsFixed(5)}, ${o.deliveryLongitude!.toStringAsFixed(5)}',
                  ),
  ...
  Future<void> _setStatus(DeliveryInfo d, String status) async {
    ...
    try {
      // FIX: removed the OUT_FOR_DELIVERY coordinate-blocking check.
      //
      // Root cause of the bug: the old code read order.deliveryLatitude
      // (the customer's DROP-OFF location) and passed it as the driver's
      // CURRENT position — semantically wrong. These are two different things:
      //   • order.deliveryLatitude/Longitude  = customer's drop-off address
      //   • currentLatitude/Longitude         = driver's live GPS position
      //
      // Additionally, the backend was rejecting OUT_FOR_DELIVERY when no
      // coordinates were provided, blocking drivers from progressing their run.
      //
      // Fix: coordinates are now omitted entirely. The backend requirement
      // has been removed from PUT /api/deliveries/:id. When you add a GPS
      // package (e.g. geolocator), pass the device location here instead.
      await _api.updateDelivery(
        id: d.id,
        status: status,
        // currentLatitude / currentLongitude — add geolocator here later
      );
  ...
  Future<void> _collectCod(DeliveryInfo d) async {
    ...
    final totalText = '${order.currency} ${order.total.toStringAsFixed(2)}';
    final yes = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('Collect cash & mark paid?'),
        content: Text(
          'Confirm you collected $totalText for order #${d.orderId}. This will mark the order as paid in the app.',
        ),
        actions: [
          TextButton(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          widget.readOnly ? 'Deliveries (view only)' : 'My deliveries',
        ),
        actions: [
          IconButton(
            tooltip: 'Refresh',
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(
                          'Orders are assigned to you by an admin. Tap "Start pickup" when you collect the bag, then update status as the run progresses.',
                          style: TextStyle(
                            color: cs.onSurfaceVariant,
                            height: 1.35,
                            fontSize: 13,
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        children: [
                          ...
                          Text(
                            'An admin will assign orders to you.',
                            style: TextStyle(
                              color: cs.onSurfaceVariant,
                              fontSize: 13,
                            ),
                          ...
                          children: [
                            ...
                            if (d.driverName != null &&
                              ...
                              Padding(
                                ...
                                child: Text(
                                  'Driver: ${d.driverName}${d.driverPhone != null ? ' · ${d.driverPhone}' : ''}',
                                  style: TextStyle(
                            ...
                            Wrap(
                              ...
                              children: [
                                ...
                                if (!widget.readOnly && d.status == 'PENDING')
                                  FilledButton.icon(
                                    onPressed: () => _setStatus(d, 'PICKED_UP'),
                                    icon: const Icon(
                                ...
                                if (!widget.readOnly &&
                                    (d.status == 'OUT_FOR_DELIVERY' ||
                                        d.status == 'PICKED_UP'))
                                  FilledButton(
                                    onPressed: () => _setStatus(d, 'DELIVERED'),
