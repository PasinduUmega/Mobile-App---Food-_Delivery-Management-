...
class _OrderManagementDashboardState extends State<OrderManagementDashboard> {
  ...
  Widget build(BuildContext context) {
    ...
    // Store owners must not CRUD orders/carts. Also treat any owner-filtered
    // view as read-only (used by the store owner shell).
    final isReadOnly =
        widget.readOnly ||
        widget.role == UserRole.storeOwner ||
...
class _OrderEditDialogState extends State<_OrderEditDialog> {
  ...
  Future<void> _save() async {
    ...
    try {
      final itemsToSave = _cart.map((e) => CartItem(
      ...
      if (widget.existing == null) {
        ...
        await widget.api.updateOrder(
          ...
          status: _status,
          items: itemsToSave.map((item) => item.toJson()).toList(),
        );
      }
