...
class ApiClient {
  ...
  Future<void> updateOrder({
    ...
    final uri = Uri.parse('$baseUrl/api/orders/$id');
    final data = <String, dynamic>{'status': ?status, 'items': ?items};
  ...
  /// Returns all paid/preparing orders that have no driver assigned yet.
  /// Calls GET /api/orders/unassigned
  Future<List<OrderSummary>> getUnassignedOrders() async {
    final uri = Uri.parse('$baseUrl/api/orders/unassigned');
    final resp = await _http.get(uri, headers: _jsonHeadersWithSession());
    final body = _decode(resp);
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw ApiException(
        body['error']?.toString() ?? 'Failed to fetch unassigned orders',
      );
    }
    final items = body['items'];
    if (items is List) {
  ...
  /// Assigns a driver to an order. The backend auto-sets order → PREPARING
  /// and creates/updates the delivery row.
  /// Calls POST /api/orders/:id/assign
  Future<DeliveryInfo> assignDriverToOrder({
    required int orderId,
    required int driverUserId,
  }) async {
    final uri = Uri.parse('$baseUrl/api/orders/$orderId/assign');
    final resp = await _http.post(
      uri,
      headers: _jsonHeadersWithSession(),
      body: jsonEncode({'driverUserId': driverUserId}),
    );
    final body = _decode(resp);
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw ApiException(
        body['error']?.toString() ?? 'Failed to assign driver to order',
      );
    }
    // Backend returns { ok: true, delivery: { ...DeliveryInfo fields } }
    final deliveryMap = body['delivery'];
    if (deliveryMap is Map<String, dynamic>) {
      return DeliveryInfo.fromJson(deliveryMap);
    }
    throw ApiException('Unexpected response from assign endpoint');
  }
  ...
  /// Fetches all deliveries assigned to a specific driver.
  /// Calls GET /api/deliveries/driver/:userId
  Future<List<DeliveryInfo>> listDriverDeliveries({required int userId}) async {
    final uri = Uri.parse('$baseUrl/api/deliveries/driver/$userId');
    final resp = await _http.get(uri, headers: _jsonHeadersWithSession());
    final body = _decode(resp);
    if (resp.statusCode < 200 || resp.statusCode >= 300) {
      throw ApiException(
        body['error']?.toString() ?? 'Failed to fetch driver deliveries',
      );
    }
    final items = body['items'];
    if (items is List) {
