...
/** Public signup roles (includes ADMIN for first admin / dev; restrict in production if needed). */
...
app.post('/api/deliveries', async (req, res) => {
  ...
  try {
    const [result] = await pool.query(
      ...
      [asInt(orderId), driverName || null, driverPhone || null]
    );
  ...
  try {
    ...
    await pool.query(
      `UPDATE deliveries SET 
        ...
        updated_at=NOW() 
       WHERE id=?`,
      [status || null, driverName || null, driverPhone || null, pickupTime || null, deliveryTime || null, lat, lng, id]
    );
